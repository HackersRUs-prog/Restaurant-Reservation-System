...
<head>
    <meta charset="utf-8">
    <title>Fine Dining - Guest Homepage</title>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-dark navbar-dark shadow-sm py-3 py-lg-0 px-3 px-lg-0">
        <a href="index2.php" class="navbar-brand d-block d-lg-none">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto mx-lg-auto py-0">
                ...
                <a href="viewcart.php" class="nav-item nav-link">My Orders (<?php echo ($cart->total_items() > 0)?$cart->total_items():0; ?>)</a>
    ...
    <div id="about" class="container-fluid pt-5">
        <div class="container">
            <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
                ...
                <h1 class="display-4 text-uppercase">Welcome Guest, <?php echo $row1['fullname']; ?></h1>
            </div>
            <div class="row gx-5">
                <div class="col-lg-5 mb-5 mb-lg-0" style="min-height: 400px;">
                    <div class="position-relative h-100">
                        <img class="position-absolute w-100 h-100" src="img/<?php echo $row1['image']; ?>" style="object-fit: cover;">
                    </div>
                </div>
...
$sql = "SELECT `food_id`,`food_name`, `food_image`, `food_category`, `food_sellingprice`, `quantity` FROM `tbl_food` WHERE `quantity` >= 0";
...
<td><button onclick="return confirm('Are you sure to order this item?')?window.location.href='cartaction.php?action=addToCart&id=<?php echo ($row['food_id']); ?>':true;" title='Add To Cart'>Add</button></td>
</tr>
<?php
}
} else { echo "0 results."; }
...
<div class="d-flex align-items-center justify-content-center justify-content-lg-start pt-5">
            ...
            <br>
            <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
                <h2 class="text-primary font-secondary">Reservations</h2>
            </div>
            <div class="tab-class text-center">
                    <div class="tab-content">
...
$sql = "SELECT `reservation_id`, `reservation_details`, `time`, `date`, `customer_id`, `created_on`, `updated_on`, `server_id`, `order_id` FROM `tbl_reservations` WHERE `customer_id` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><?php echo($row["reservation_details"]); ?></td>
...
<div class="d-flex align-items-center justify-content-center justify-content-lg-start pt-5">
            ...
            <br>
            <br>
            <div id="orders" class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
...
$sql = "SELECT `order_id`, `customer_id`, `server_id`, `created_on`, `updated_on`, `order_total`, `payment_method`, `order_status` FROM `tbl_order` WHERE `customer_id` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
$sql = "SELECT `tbl_orderdetails`.`orderdetails_id`, `tbl_orderdetails`.`food_name`, `tbl_orderdetails`.`food_id`, `tbl_orderdetails`.`created_on`, `tbl_orderdetails`.`price`, `tbl_orderdetails`.`order_id`, `tbl_orderdetails`.`quantity`, `tbl_order`.`order_id` FROM `tbl_orderdetails` JOIN `tbl_order` ON `tbl_orderdetails`.`order_id` = `tbl_order`.`order_id` WHERE `tbl_order`.`customer_id` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<br>
<div class="d-flex align-items-center justify-content-center justify-content-lg-start pt-5">
                        <a href="" onclick="printData3();" class="btn btn-primary border-inner py-3 px-5 me-5">Print</a>
                    </div>
                </div>
            </div>
        </div>
                  ...
                  <form action="insertion.inc.php" method="POST" enctype="multipart/form-data"> 
                        <div class="row g-3">
                            <div class="col-sm-6">
                                ...
                                <input type="hidden" name="uid" value="<?php echo $filter; ?>" required>
                            </div>
                            <div class="col-sm-6">
                           ...
                           <div class="col-sm-12">
                            ...
                            <div class="col-sm-12">
                                <button class="btn btn-primary border-inner w-100 py-3" type="submit" name="upg">Update </button>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="row gx-5">
                        ...
                        <div class="col-lg-4 col-md-12 pt-0 pt-lg-5 mb-5">
                            ...
                            <div class="d-flex flex-column justify-content-start">
                                <a class="text-secondary mb-2" href="index2.php"><i class="bi bi-arrow-right text-primary me-2"></i>Home</a>
