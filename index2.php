...
<head>
    <meta charset="utf-8">
    <title>Fine Dining - Server Homepage</title>
    ...
    <div id="about" class="container-fluid pt-5">
        <div class="container">
            <div class="section-title position-relative text-center mx-auto mb-5 pb-3" style="max-width: 600px;">
                ...
                <h1 class="display-4 text-uppercase">Welcome Server, <?php echo $row['fullname']; ?></h1>
            </div>
            <div class="row gx-5">
...
$sql = "SELECT `food_id`,`food_name`, `food_image`, `created_on`, `updated_on`, `food_category`, `food_sellingprice`, `quantity` FROM `tbl_food`";
...
$sql = "SELECT `reservation_id`, `reservation_details`, `time`, `date`, `customer_id`, `created_on`, `updated_on`, `server_id`, `order_id` FROM `tbl_reservations` WHERE `server_id` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
$sql = "SELECT `order_id`, `customer_id`, `server_id`, `created_on`, `updated_on`, `order_total`, `payment_method`, `order_status` FROM `tbl_order` WHERE `server_id` = '$filter' OR `order_status` = 'Pending'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
                ...
                <div class="col-lg-6">
                  <form action="insertion.inc.php" method="POST" enctype="multipart/form-data"> 
                        <div class="row g-3">
                            ...
                            <div class="col-sm-12">
                                <button class="btn btn-primary border-inner w-100 py-3" type="submit" name="ups">Update </button>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <form action="insertion.inc.php" method="POST" enctype="multipart/form-data"> 
                        <div class="row g-3">
                            <div class="col-sm-6">
                                <select name="oid" required class="form-control bg-light border-0 px-4" style="height: 55px;">
                                  <option selected disabled value="0">Select An Order ID</option>
                                     <?php
                                ...
                                <input type="hidden" name="sid" value="<?php echo $filter; ?>" required>
                            </div>
                            <div class="col-sm-12">
                                <button class="btn btn-primary border-inner w-100 py-3" type="submit" name="completeorder">Complete </button>
                            </div>
                        </div>
                    </form>
                </div>
